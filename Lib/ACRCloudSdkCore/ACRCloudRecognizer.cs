...
namespace ACRCloudSdkCore
{
    ...
    public partial class ACRCloudRecognizer : IDisposable
    {
        ...
        private static readonly KeyValuePair<string?, string?> DefaultDataTypeContent =
            new KeyValuePair<string?, string?>("data_type", DefaultDataType);

        private static readonly KeyValuePair<string?, string?> DefaultSignVersionContent =
            new KeyValuePair<string?, string?>("signature_version", DefaultSignVersion);

        public ACRCloudRecognizer(ACRCloudOptions options) => Options = options;
        ...
        private async Task<ACRCloudRecognizeResult?>
            CreateResultAsync(Task<HttpResponseMessage> response) // Suppress all CS8602/CS8604 in JToken[string] / JToken.ToObject<T>
        {
#if NETSTANDARD2_0 || NETFRAMEWORK
...
#else
            JsonElement root = await response.GetObjectAsync<JsonElement>(), status = root.GetProperty("status");
            switch (status.GetProperty("code").GetInt32())
            {
                case 0:
                {
                    JsonElement metadata = root.GetProperty("metadata"), music = metadata.GetProperty("music")[0];
                    return new ACRCloudRecognizeResult(
                        ...
                        music.TryGetProperty("play_offset_ms", out JsonElement playOffset)
                            ? TimeSpan.FromMilliseconds(playOffset.GetInt32())
                            : default(TimeSpan?),
                        music.TryGetProperty("duration_ms", out JsonElement duration)
                            ? TimeSpan.FromMilliseconds(duration.GetInt32())
                            : default(TimeSpan?),
