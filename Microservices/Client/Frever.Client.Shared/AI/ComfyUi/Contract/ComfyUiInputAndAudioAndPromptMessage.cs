...
public class ComfyUiInputAndAudioAndPromptMessage : IComfyUiMessage
{
    ...
    public List<long> ContextValues { get; set; }
    public int AudioStartTime { get; set; }
    public int AudioDuration { get; set; } // max 15 seconds
    ...
    public void Enrich(string env, string s3Bucket, long groupId)
    {
        ...
        AudioS3Bucket = string.IsNullOrWhiteSpace(AudioS3Bucket) ? s3Bucket : AudioS3Bucket;
    }
    ...
    public void Validate()
    {
        ...
        if(string.IsNullOrWhiteSpace(AudioS3Bucket) && string.IsNullOrWhiteSpace(PromptText))
            throw new ArgumentException("AudioS3Bucket or PromptText required");

        if(!string.IsNullOrWhiteSpace(AudioS3Bucket) && string.IsNullOrWhiteSpace(AudioS3Key))
            throw new ArgumentException("AudioS3Bucket and AudioS3Key required");
    }
